...
const WalletContext = React.createContext<Wallet>({
  ...
  connect: (connector: WalletConnector, args?: Record<string, any>) =>
    Promise.reject(),
...
const WalletProvider: React.FunctionComponent<any> = (props) => {
      ...
      async function onError(error: Error) {
        ...
        if (error instanceof NoEthereumProviderError) {
          ...
          await window.ethereum.request({
            ...
            params: [{ chainId: `0x${process.env.REACT_APP_WEB3_CHAIN_ID}` }],
          });
  ...
  return (
    <WalletContext.Provider value={value}>
      <ConnectWalletModal
        show={walletsModal}
        onClose={() => setWalletsModal(false)}
      />
      <InstallMetaMaskModal
        show={installMetaMaskModal}
        onClose={() => setInstallMetaMaskModal(false)}
      />
      <UnsupportedChainModal
        show={unsupportedChainModal}
        onClose={() => setUnsupportedChainModal(false)}
      />
      {props.children}
...
const Web3WalletProvider: React.FunctionComponent<any> = (props) => {
  return (
